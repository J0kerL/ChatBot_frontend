<template>
  <div class="ai-list-page">
    ...
    <section class="ai-list">
      ...
      <div class="ai-list-content custom-scrollbar">
        <div v-if="loading" class="loading">加载中...</div>
        
        <div v-else-if="aiCharacters.length > 0" class="ai-grid">
          <div 
            v-for="ai in aiCharacters" 
            :key="ai.id"
            class="ai-card"
            @click="goToDetail(ai.id)"
          >
            <div class="ai-avatar">
              <img :src="ai.avatar || defaultAvatar" :alt="ai.name" />
            </div>
            <div class="ai-info">
              <h3>{{ ai.name }}</h3>
              <p class="relationship">{{ ai.relationship }}</p>
              <div class="tags" v-if="ai.personalityTags && ai.personalityTags.length">
                <span v-for="tag in ai.personalityTags.slice(0, 3)" :key="tag" class="tag">
                  {{ tag }}
                </span>
              </div>
            </div>
            <div class="ai-actions">
              <button class="btn-edit" @click.stop="goToEdit(ai.id)" title="编辑">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                  <path d="M3 17.25V21H6.75L17.81 9.94L14.06 6.19L3 17.25ZM20.71 7.04C21.1 6.65 21.1 6.02 20.71 5.63L18.37 3.29C17.98 2.9 17.35 2.9 16.96 3.29L15.13 5.12L18.88 8.87L20.71 7.04Z" fill="currentColor"/>
                </svg>
              </button>
              <button class="btn-delete" @click.stop="handleDelete(ai)" title="删除">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
        ...
        <div v-else class="empty-list">
          ...
          <p>暂无AI机器人</p>
          <button class="btn-primary" @click="goToAdd">创建第一个AI机器人</button>
        </div>
      </div>
...
const loadAiCharacters = async () => {
  try {
    ...
    aiCharacters.value = result.records || []
...
const handleDelete = async (ai) => {
  const confirmed = await showDeleteConfirm(`确定要删除 "${ai.name}" 吗？`)
  if (!confirmed) return
  
  try {
    await deleteAiCharacter(ai.id)
